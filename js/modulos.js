...
export async function cargarTablaModulos() {
    ...
    try {
        ...
        modulosArray.forEach(data => {
            ...
            // =========================================================
           const cantidadEvaluacion = data.cantidadPreguntas || 0;

            // 1. Definimos la imagen por defecto en formato SVG
           const defaultImg = "data:image/svg+xml;base64,";
   ...
   // ==========================================
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function () {
            ...
            filasTabla.forEach(fila => {
                // Ignorar filas que no tienen datos reales (ej. mensaje de "No tienes módulos")
                if (fila.cells.length <= 1) return;

                // CORRECCIÓN AQUÍ: 
                // cells[1] es el ID del Módulo
                // cells[2] es la columna con el Nombre, Descripción, Docente y Categoría
                const idModulo = fila.cells[1].textContent.toLowerCase();
                const detallesModulo = fila.cells[2].textContent.toLowerCase();

                // Evaluamos si el término de búsqueda está en el ID o en toda la celda de detalles
                if (idModulo.includes(terminoBusqueda) || detallesModulo.includes(terminoBusqueda)) {
                    fila.style.display = '';
                } else {
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            ...
            const cantidadPreguntas = parseInt(document.getElementById('mod-cantidad-preguntas').value) || 0;
...
window.abrirModalEditarModulo = async (idDocumento, idTecnico, nombreModulo) => {
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            document.getElementById('edit-mod-cantidad-preguntas').value = data.cantidadPreguntas || 0;
...
export function configurarModalEdicion() {
    ...
    if (formEditModule) {
        formEditModule.addEventListener('submit', async (e) => {
            ...
            const cantidadPreguntas = parseInt(document.getElementById('edit-mod-cantidad-preguntas').value) || 0;
...
export async function cargarCategoriasSelect(selectId, categoriaAsignada = "") {
    const select = document.getElementById(selectId);
    if (!select) return;
